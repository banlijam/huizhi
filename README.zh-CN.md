...
账本模块实现**复式记账法** — 每笔交易至少更新两个账户，所有金额变动的总和始终为零。

平台作为**代收款方**：资金存放在平台的 Transfi 账户中，账本跟踪每个商户的应得金额以及平台自身的收入。

### 账户类型

| 类型 | 余额方向 | 描述 |
|------|:--------:|------|
| `ASSET_AVAILABLE` | 正数 (+) | 商户可用余额（资产） |
| `LIABILITY_CUSTODY` | 负数 (-) | 托管负债 — 负余额代表平台欠商户的金额 |
| `PLATFORM_INCOME` | 正数 (+) | 平台手续费/收入 |
| `PLATFORM_COST` | 正数 (+) | 平台成本支出（如 KYC/KYT 外部查询供应商费用） |

### 业务流程

**流程 1：客户付款 / 充值（1000，7% 手续费）**

客户支付 1000 → 平台通过 Transfi 收款 → 商户获得 93%（930），平台收取 7%（70）作为手续费。

| 账户 | 金额 | 说明 |
|------|:----:|------|
| `ASSET_AVAILABLE` | +930 | 商户可用余额增加（93% 净额） |
| `PLATFORM_INCOME` | +70 | 平台手续费收入（7%） |
| `LIABILITY_CUSTODY` | -1000 | 托管负债增加（余额向负方向变化） |

> 合计：930 + 70 - 1000 = 0 ✓

**流程 2：商户提现 / 出金（500）**

商户提现 500 到钱包 → 平台调用 Transfi 发送资金。

| 账户 | 金额 | 说明 |
|------|:----:|------|
| `ASSET_AVAILABLE` | -500 | 商户可用余额减少 |
| `LIABILITY_CUSTODY` | +500 | 托管负债减少（余额向正方向变化） |

> 合计：-500 + 500 = 0 ✓

**流程 3：外部查询成本（KYC/KYT，平台支付供应商 0.8）**

当平台需要进行外部查询时，直接从自身收入中向供应商付款。

| 账户 | 金额 | 说明 |
|------|:----:|------|
| `PLATFORM_COST` | +0.8 | 供应商成本（费用支出） |
| `PLATFORM_INCOME` | -0.8 | 平台收入减少用于支付供应商 |
